    ...
    if (pay[i].checked) {
      if (pay[i].value === 'SBI Card') {
        ...
        for (var j = 0; j < _cart.length; j++) {
          var carts = new Array();
          carts[j] = _cart[j];
          console.log(_cart[j].innerText, user, address, pay[i].value);
        }
      }
    }
  }
